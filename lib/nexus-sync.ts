...
export const emitNexusEvent = (event: Omit<NexusEvent, "id" | "time">) => {
    ...
    try {
        ...
        const events: NexusEvent[] = existingEvents ? JSON.parse(existingEvents) : [];
        events.unshift(newEvent); // Add to beginning
        // Keep last 10 events
        const trimmedEvents = events.slice(0, 10);
        localStorage.setItem("NEXUS_DASHBOARD_EVENTS", JSON.stringify(trimmedEvents));

        // Dispatch custom event for same-page listeners
        window.dispatchEvent(new CustomEvent("nexus-dashboard-sync", { detail: newEvent }));
    } catch (e) {
